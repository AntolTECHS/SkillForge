...
import { Home, BookOpen, MessageSquare, Award, Users, UserPlus, Settings } from "lucide-react";
...
{ name: "Settings", icon: <Settings size={20} />, path: "/instructor/settings" },
...
>
{/* NAVIGATION */} <div> <nav className="p-4 mt-2 space-y-1 overflow-y-auto">
{mainMenu.map((item) => (
<NavLink
key={item.name}
to={item.path}
...
>
{item.icon} <span>{item.name}</span> </NavLink>
))} </nav> </div>
